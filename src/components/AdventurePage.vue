				...
				<b-col>
					...
					<ol>
						<li v-for="option in page.options" :key="option.click">
							<a href="#" v-on:click="handleClick(option.optionClick)">{{option.optionText}}</a>
						</li>
						<li>
							<a href="#" v-on:click="save()">Save Progress</a>
						</li>
					</ol>
				</b-col>
				<b-col>
					<img class="adventure_picture" :src="'' + page.path"/>
				</b-col>
			</b-row>
		</b-container>
	</div>
...
export default {
  ...
  methods: {
	async loadPage() {
		let response = await axios.get("/api/pages/" + this.pageName);
		this.page = response.data;
	},
	async handleClick(option) {
		await this.$parent.handleClick(option);
		this.loadPage();
	},
	save() {
		this.$parent.save();
